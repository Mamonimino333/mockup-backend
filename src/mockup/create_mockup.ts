...
async function perspectiveTransform(params) {
  const { template, artwork, out, coord } = params;
  ...
  const transform = `convert "${template}" -alpha transparent \\( "${artwork}" +distort perspective "${coord}" \\) -background transparent -layers merge +repage "${out}"`
...
async function generateMockup(params) {
  const { artwork, template, displacementMap, lightingMap, adjustmentMap, mask, out, coord } = params;
  const tmp = path.join(os.tmpdir(), `${Math.random().toString(36).substring(7)}.mpc`);
  await addBorder({ artwork, out: tmp });
  await perspectiveTransform({ template, artwork: tmp, out: tmp, coord: coord });
